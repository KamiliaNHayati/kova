...
(define-public (register-service
  ...
  (let ((count (default-to u0 (map-get? service-count contract-caller))))
...
;; Delete a service and shift remaining services down to fill the gap
(define-public (delete-service (index uint))
  (let ((count (default-to u0 (map-get? service-count contract-caller))))
    (asserts! (is-some (map-get? user-services { owner: contract-caller, index: index })) ERR-SERVICE-NOT-FOUND)
    (asserts! (< index count) ERR-SERVICE-NOT-FOUND)
    ;; Shift services down to fill the gap (up to MAX-SERVICES-PER-USER)
    (and (< (+ index u1) count)
         (match (map-get? user-services { owner: contract-caller, index: (+ index u1) })
           svc1 (map-set user-services { owner: contract-caller, index: index } svc1) false))
    (and (< (+ index u2) count)
         (match (map-get? user-services { owner: contract-caller, index: (+ index u2) })
           svc2 (map-set user-services { owner: contract-caller, index: (+ index u1) } svc2) false))
    (and (< (+ index u3) count)
         (match (map-get? user-services { owner: contract-caller, index: (+ index u3) })
           svc3 (map-set user-services { owner: contract-caller, index: (+ index u2) } svc3) false))
    (and (< (+ index u4) count)
         (match (map-get? user-services { owner: contract-caller, index: (+ index u4) })
           svc4 (map-set user-services { owner: contract-caller, index: (+ index u3) } svc4) false))
    ;; Delete the last slot and decrement count
    (map-delete user-services { owner: contract-caller, index: (- count u1) })
    (map-set service-count contract-caller (- count u1))
    (ok true)
  )
)
